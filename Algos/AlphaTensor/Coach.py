...
class Coach:
    ...
    def play(self) -> List[Tuple[State, np.ndarray, float]]:
        ...
        while True:
            ...
            if num_steps >= self.max_num_steps or self.environment.is_terminal(state):     # max no of step are 8
                cumulative_reward += self.environment.get_final_reward(state)
                break

        examples = [(s, pi, cumulative_reward - r) for (s, pi, _), r in zip(train_examples, cumulative_rewards)]
        return examples, cumulative_reward

    def learn(self):
        cumulative_reward_logger = pl.loggers.TensorBoardLogger(save_dir=self.logs_dir, name='cumulative_rewards') #creating directory for logging of cumulative rewards
